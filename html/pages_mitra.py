...
def build_project_detail():
    tabs = ['Overview', 'Progress', 'S-Curve', 'Dokumen', 'Aktivitas']
    tab_html = ''.join(f'<a href="#" class="tab{" is-active" if i==0 else ""}" data-tab="t{i}">{t}</a>' for i, t in enumerate(tabs))
    ...
    def progress_table(rows):
        trs = ''.join(f'''<tr><td style="padding-left:20px">{name}</td><td class="mono">{bobot}%</td>
          <td style="padding-right:20px"><div class="flex items-center gap-8"><div class="pbar" style="flex:1;min-width:100px"><span style="width:{prog}%"></span></div><span class="mono" style="font-weight:600">{prog}%</span></div></td></tr>''' for name, bobot, prog in rows)
        return f'''<table class="tbl" style="margin:0"><thead><tr><th style="padding-left:20px">AKTIVITAS</th><th>BOBOT</th><th style="padding-right:20px">PROGRESS</th></tr></thead><tbody>{trs}</tbody></table>'''
    def progress_category(name, rows, avg):
        return f'''<div style="margin-bottom:20px">
          <div class="flex justify-between items-center" style="margin-bottom:10px">
            <div style="font:700 15px var(--font-head)">{name}</div>
            <span class="sub">{len(rows)} item &middot; avg actual <b style="color:var(--slate-900)">{avg}%</b></span>
          </div>
          <div class="card" style="padding:0">{progress_table(rows)}</div>
        </div>'''
    eng_rows = [('Detailed Engineering Design', 15, 100), ('Single Line Diagram', 8, 100)]
    proc_rows = [('Solar Panel', 25, 80), ('Inverter', 15, 80), ('Mounting System', 10, 60)]
    constr_rows = [('Instalasi Panel &amp; Struktur Mounting', 20, 0), ('Commissioning &amp; Testing', 12, 0)]
    progress_tab = card('Progress per Item', f'''
      {progress_category('Engineering', eng_rows, 100)}
      {progress_category('Procurement', proc_rows, 59)}
      {progress_category('Construction', constr_rows, 0)}
      <p class="sub" style="margin-top:4px">*Nama item Construction belum dirinci pada desain Figma &ndash; ditampilkan sebagai perkiraan agar tabel tetap konsisten.</p>''')

    # ---- Tab 2: S-Curve ----
    scurve_stats = f'''<div class="kpi-row" style="grid-auto-columns:1fr;margin-bottom:20px">
      <div class="stat-box"><div class="n">48%</div><div class="l">Planned Today</div></div>
      <div class="stat-box stat-box--red"><div class="n">42%</div><div class="l">Actual Today</div></div>
      <div class="stat-box stat-box--red"><div class="n">-6%</div><div class="l">Variance</div></div>
      <div class="stat-box"><div class="n">7 minggu</div><div class="l">Sisa Minggu</div></div>
    </div>'''
    scurve_legend = f'''<div class="flex gap-24 items-center" style="margin-bottom:12px">
      <span class="flex items-center gap-8 sub"><svg width="22" height="10"><line x1="0" y1="5" x2="22" y2="5" stroke="#94A3B8" stroke-width="2" stroke-dasharray="4 3"/></svg>Planned (Baseline)</span>
      <span class="flex items-center gap-8 sub"><svg width="22" height="10"><line x1="0" y1="5" x2="22" y2="5" stroke="#16A34A" stroke-width="2.5"/></svg>Actual</span>
      <span class="flex items-center gap-8 sub"><span style="width:14px;height:14px;background:#FECDD3;display:inline-block;border-radius:3px"></span>Behind schedule</span>
    </div>'''
    scurve_svg = '''<svg viewBox="0 0 760 240" style="width:100%;height:auto">
      <line x1="50" y1="20" x2="50" y2="200" stroke="#E2E8F0"/>
      <line x1="50" y1="200" x2="730" y2="200" stroke="#E2E8F0"/>
      <g stroke="#F1F5F9"><line x1="50" y1="155" x2="730" y2="155"/><line x1="50" y1="110" x2="730" y2="110"/><line x1="50" y1="65" x2="730" y2="65"/><line x1="50" y1="20" x2="730" y2="20"/></g>
      <g font-size="10" fill="#94A3B8" font-family="Inter,sans-serif" text-anchor="end">
        <text x="44" y="203">0%</text><text x="44" y="158">25%</text><text x="44" y="113">50%</text><text x="44" y="68">75%</text><text x="44" y="23">100%</text>
      </g>
      <g font-size="10" fill="#94A3B8" font-family="Inter,sans-serif" text-anchor="middle">
        <text x="50" y="216">W1</text><text x="220" y="216">W4</text><text x="333" y="216">W6</text><text x="560" y="216">W10</text><text x="730" y="216">W13</text>
      </g>
      <polygon points="220,164 260,140 300,125 333,113.6 333,124.4 300,142 260,160 220,173" fill="#FECDD3" opacity="0.7"/>
      <path d="M50,196.4 C120,190 180,175 220,164 C260,150 300,125 333,113.6 C400,95 480,60 560,47 C620,35 680,25 730,20" fill="none" stroke="#94A3B8" stroke-width="2" stroke-dasharray="5 4"/>
      <path d="M50,198.2 C120,193 180,180 220,173 C260,160 300,138 333,124.4" fill="none" stroke="#16A34A" stroke-width="2.5"/>
      <line x1="333" y1="20" x2="333" y2="200" stroke="#CBD5E1" stroke-width="1.5" stroke-dasharray="3 3"/>
      <text x="333" y="12" font-size="10" font-weight="700" fill="#64748B" font-family="DM Sans,sans-serif" text-anchor="middle">TODAY</text>
      <circle cx="333" cy="113.6" r="4" fill="#fff" stroke="#94A3B8" stroke-width="2"/>
      <circle cx="333" cy="124.4" r="4.5" fill="#16A34A"/>
    </svg>'''
    scurve_tab = card('S-Curve Progress Monitoring', f'''
      <p class="sub" style="margin-bottom:16px">PRJ-2024-0019 &middot; RSUD Dr. Soetomo &middot; 200 kWp &middot; {dot_badge('At Risk','amber')}</p>
      {scurve_stats}{scurve_legend}{scurve_svg}''')

    # ---- Tab 3: Dokumen ----
    doc_rows = [
        ('DED Engineering', 'Technical Doc', '2024-09-05'),
        ('Progress Report W8', 'Progress Report', '2024-09-08'),
        ('Foto Kemajuan W8', 'Evidence', '2024-09-08'),
    ]
    doc_trs = ''.join(f'''<tr><td style="padding-left:20px" class="strong">{name}</td><td>{tipe}</td><td>{tgl}</td>
      <td style="padding-right:20px"><a href="#" style="font:600 13px var(--font-head);color:var(--green-700)">{icon('↓',13)} Unduh</a></td></tr>''' for name, tipe, tgl in doc_rows)
    dokumen_tab = card('Dokumen Proyek', f'''<div class="card" style="padding:0"><table class="tbl" style="margin:0">
      <thead><tr><th style="padding-left:20px">DOKUMEN</th><th>TIPE</th><th>TANGGAL</th><th style="padding-right:20px">AKSI</th></tr></thead>
      <tbody>{doc_trs}</tbody></table></div>''')

    # ---- Tab 4: Aktivitas ----
    activity = [
        ('#DBEAFE', '+', 'Proyek dimulai setelah kontrak ditandatangani', 'ICONGreen &middot; 2024-09-10'),
        ('#DCFCE7', '✓', 'DED Engineering diselesaikan dan diupload', 'Alpha Solar &middot; 2024-09-05'),
        ('#FEF3C7', '↑', 'Progress Report W8 disubmit: Actual 42%', 'Alpha Solar &middot; 2024-09-08'),
        ('#DCFCE7', '✓', 'Progress W8 diverifikasi dan diterima', 'ICONGreen &middot; 2024-09-09'),
    ]
    act_items = ''.join(f'''<div class="timeline__item"><div class="timeline__dot" style="background:{bg}">{icon(ic,13)}</div>
      <div class="timeline__body"><div class="t">{t}</div><div class="d">{d}</div></div></div>''' for bg, ic, t, d in activity)
    aktivitas_tab = card('Riwayat Aktivitas', f'<div class="mt-16">{act_items}</div>')

    other_tabs = f'''<div data-tab-panel="t1" data-tabs-for="prj" hidden>{progress_tab}</div>
      <div data-tab-panel="t2" data-tabs-for="prj" hidden>{scurve_tab}</div>
      <div data-tab-panel="t3" data-tabs-for="prj" hidden>{dokumen_tab}</div>
      <div data-tab-panel="t4" data-tabs-for="prj" hidden>{aktivitas_tab}</div>'''
...
def build_update_progress():
    ...
    proc_panel = item_card('Solar Panel', 80, 80, '+0%') + item_card('Inverter', 80, 80, '+0%') + item_card('Mounting System', 60, 60, '+0%')
    proc_panel += f'<div class="card mt-16" style="text-align:center;color:var(--slate-500);cursor:pointer">+ Tambah Item Procurement</div>'
    proc_panel += f'<p class="sub mt-16">*Diisi ulang dari data Progress tab agar tetap konsisten &ndash; sesuaikan angka bila progress terbaru berbeda.</p>'

    constr_panel = item_card('Instalasi Panel &amp; Struktur Mounting', 20, 0, '+0%') + item_card('Commissioning &amp; Testing', 0, 0, '+0%')
    constr_panel += f'<div class="card mt-16" style="text-align:center;color:var(--slate-500);cursor:pointer">+ Tambah Item Construction</div>'
    constr_panel += f'<p class="sub mt-16">*Nama &amp; bobot item Construction tidak dirinci pada desain Figma untuk layar ini &ndash; ditampilkan sebagai perkiraan yang konsisten dengan tab Progress, bukan data resmi dari Figma.</p>'

    other_panels = f'''<div data-tab-panel="t1" data-tabs-for="upd" hidden>{proc_panel}</div>
      <div data-tab-panel="t2" data-tabs-for="upd" hidden>{constr_panel}</div>'''
...
def build_company_profile():
    ...
    for i, t in enumerate(tabs[1:], start=1):
        placeholder = f'<p class="sub">{t} PT Alpha Solar Energi belum tersedia di desain Figma &ndash; hanya tab Company Information yang dirancang untuk layar ini.</p>'
